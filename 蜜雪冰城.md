...
```js
//day 11    由于无法获取用户手机号，所以手机号的部分功能没有实现 
1. 云开发-内容管理-门店-新建模型 - 展示名称：客户 - 数据库名：user - 创建时间：created_time - 更新时间 updated_time
2. 点击客户模型-添加电话号码字段- 展示名称：电话号码 - 数据库字段名：phone_number - 是否必须：true
// 文章页面
3.云开发-内容管理-门店-新建模型 - 展示名称：自定义页面 - 数据库名：page - 创建时间：created_time - 更新时间 updated_time
4. 点击自定义页面模型-添加单行字符串字段- 展示名称：唯一标识 - 数据库字段名：code - 是否必须：true
5. 点击自定义页面模型-添加单行字符串字段- 展示名称：标题 - 数据库字段名：title - 是否必须：true
6. 点击自定义页面模型-添加富文本字段- 展示名称：内容 - 数据库字段名：content - 是否必须：true
7. 自定义页面 - 新建 -唯一标识:privacy - 标题:隐私政策 - 内容随便填写
8. 创建新页面 pages/custom-page/index
9. 注意修改page的数据库权限 云开发 - 数据库 - page - 所有人可读，仅创建者创建读写
```
...
2. 给隐私政策设置点击事件，并且传入参数 pages/me/index.wxml
...
3. pages/me/index.js 跳转至隐私政策页面，并且用query方式传参
...
4. pages/custom-page/index.js   接收传入的值，并且获取数据
...
5. pages/custom-page/index.wxml    渲染页面

```html
<rich-text class="custom-page" nodes="{{page.content}}">

</rich-text>
```

6. pages/custom-page/index.scss 

```scss
/* pages/custom-page/index.wxss */
.custom-page{
  padding: 0 30rpx;
  width: 100vw;
  box-sizing: border-box;
...
```js
//设置唯一标识的目的：后续对其更改或者删除，可以根据唯一标识去操作，如果是用Id，需要把小程序弄一遍
1. 云开发-内容管理-内容集合-自定义页面-创建自定义页面内容-唯一标识：agreements - 标题：用户服
务协议 - 内容自己填写
2. 云开发-内容管理-内容集合-自定义页面-创建自定义页面内容-唯一标识：xuke - 标题：经营信息公示 - 内容自己填写
```

1. pages/me/index.wxml   只需要绑定一样的时间，将唯一标识做一下更改其他都一样即可，只需要更改标识即可更新数据渲染页面
...
```js
1. 支持MobX辅助库，改造用户模型
2. https://github.com/wechat-miniprogram/mobx-miniprogram-bindings
3. 进入 miniprogram 文件夹  npm install --save mobx-miniprogram mobx-miniprogram-bindings
4. 但是这样做的是会出现在 node_modules 文件夹中，如果要放使用就要放在 miniprogram_npm 里面，我们点击工具-构建 npm 即可
```
...
```js
1. 绑定门店信息实现思路，这里采用在点击进入菜单的时候，将门店数据传入，然后点击进入事件，使用navigateTo进入菜单页面，然后使用 query 传参，在传参之前需要转成 JSON 的形式，到菜单界面视同JSON.stringfy()进行转化回来
2. // pages/store/index.wxml
...
5. // pages/menu/index.wxml   渲染页面数据
```
...
1. components/sidebar/index.wxml
...
<scroll-view class="sidebar" scroll-y show-scrollbar="{{false}}" enhanced>
  <view class="sidebar-item {{index == current ? 'active':''}}" wx:for="{{list}}" wx:key="index" bindtap="switch" data-current="{{ index }}">
  <image class="item-icon" src="../../assets/images/menu-icon.png"></image>
  <view class="item-label">双旋冰淇淋</view>
  <!-- 选中之后左侧的红线 -->
...
2. components/sidebar/index.scss

```scss
/* components/siderbar/index.wxss */
@import '../../style//theme.scss';
.sidebar{
  width: 100%;
  height: 1148rpx;
...
4. components/sidebar/index.json
...
5. 使用组件 pages/menu/index.json

```json
{
  "usingComponents": {
    "tabs":"../../components/tabs",
    "siderbar":"../../components/siderbar/index"
  },
  "navigationStyle": "custom"
}
```

6.  pages/menu/index.wxml
...
```js
1. 抽离出去首页的轮播图，放在组件中，商品页面中也要使用
2. 样式放进从首页的地方放进组件中
3. 下面轮播图片切换方法放进组件的方法中
4. 组件引入，并且将宽度高度传入，组件的宽高由外部使用组件的地方决定
```

1. components/force-swiper/index.wxml  
...
2. components/force-swiper/index.scss
...
3. components/force-swiper/index.js
...
4. components/force-swiper/index.json
...
5. 首页部分使用组件，并且传入参数，样式 round是切角   pages/index/index.wxml
...
```js
1. 样式和方法都迁移进组件中，首页不再包含轮播图的样式和方法了
```

7. 菜单栏使用组件  pages/menu/index.wxml
...
<view class="menu">
  <!-- 边侧栏组件 -->
  <siderbar style="width: 200rpx;height: 200rpx;"></siderbar>
  <view class="menu-list">
    <force-swiper 
    swiperList="{{ swiperList }}"
    ></force-swiper>
...
8. pages/menu/index.scss

```scss
// 轮播图组件中的样式默认是菜单栏中的大小，所以这里不需要写样式
@import '../../style/theme.scss';
...
.menu{
  ...
  // grid布局
  display: grid;
  // 左侧侧边栏宽度是160rpx 1fr 把其余部分全部占满
  grid-template-columns 193rpx 1fr;
  // 两个轨道之间的间隔
  gap:20rpx;
  // 给右侧内容距离屏幕间隔32rpx
  padding-right: 32rpx; 
...
9. pages/menu/index.js    获取轮播图使用的图片
...
```json
{
  "usingComponents": {
    "tabs":"../../components/tabs",
    "siderbar":"../../components/siderbar/index",
    "force-swiper":"../../components/force-swiper/index"
  },
  "navigationStyle": "custom"
}
```
        ...
        <view class="section-list">
          <view class="item">
            ...
            <view class="info">
              ...
              <view class="description">咖啡遇到冰淇淋，清爽更提神</view>
...
.menu{
  ...
  padding-right: 32rpx; 
  .menu-list{
    // 菜单栏右侧的高度，配合外面的 scorll-view 可以有滚动效果 
    height: 1148rpx;
    .list{
      margin-top: 44rpx;
      .section{
...
```js
1. 新建模型：展示名称：商品 - 数据库名：goods - 创建时间：created_time - 更新时间：updated_time
2. 单行字符串 - 展示名称：名称 - 数据库字段名：name - 是否必需：true
3. 添加【多行字符串】字段 - 展示名称：商品简介 - 数据库字段名：description - 是否必需：false
4. 添加【图片】字段 - 展示名称：商品图片 - 数据库字段名：cover -资源链接格式:HTTPS - 是否必需：false
5. 添加【数字】字段- 展示名称：价格 - 数据库字段名：price - 最小值:0 - 是否必需：true
// 可以添加多个所以是复数 specs_categories
6. 商品 - 内容类型 - 添加【关联】字段 - 展示名称：规格分类 - 数据库字段名：specs_categories - 关联内容:规格分类 -展示字段：标识 - 是否关联多项：true
7. 商品 - 内容类型 - 添加【关联】字段 - 展示名称：商品分类 - 数据库字段名：category - 关联内容:商品分类 -展示字段：名称 - 是否关联多项：false - 是否必须：true // 一个商品只能关联一个分类
```

```js
// 商品需要关联的模型之一：规格模型
1. 新建模型：展示名称：规格分类 - 数据库名：specs_category - 创建时间：created_time - 更新时间：updated_time
2. 添加【单行字符串】字段 - 展示名称：标识 - 数据库字段名：label  - 是否必需：true
3. 添加【数字】字段 - 数据库字段名：maxUnit -默认值:1 -  最小值:1 - 是否必需：true
4. 添加【关联】字段 - 展示名称：规格 - 数据库字段名：specs - 关联内容:规格 -展示字段：标识 - 是否关联多项：true    // 多个只能关联一个
```

```js
// 商品需要关联的模型之一：规格
1. 新建模型：展示名称：规格 - 数据库名：specs - 创建时间：created_time - 更新时间：updated_time
2. 添加【单行字符串】字段 - 展示名称：标识 - 数据库字段名：label  - 是否必需：true
3. 添加【数字】字段 - 数据库字段名：price -默认值:0 -  最小值:0 - 是否必需：true
```

```js
// 商品分类模型
1. 新建模型：展示名称：商品分类 - 数据库名：goods_category - 创建时间：created_time - 更新时间：updated_time
2. 添加【单行字符串】字段 - 展示名称：名称 - 数据库字段名：name  - 是否必需：true
3. 添加【图片】字段 - 展示名称：图标 - 数据库字段名：icon -资源链接格式:HTTPS - 是否必需：true
```

```js
//1. 创建内容 
创建【商品分类】内容 - 名称：双炫冰淇淋 - 图片：带榜首的图片(图片暂时随意)
创建【商品分类】内容 - 名称：奶昔 - 图片：带榜首的图片
//2. 创建【商品】内容
名称：摩天脆脆 - 商品简介：一口双色口味，口味颜值加倍 - 商品图片：(暂时随意) - 价格：3 - 商品分类：双炫冰淇淋
// 3. 创建【规格分类】内容
辨识：冰淇淋口味 - 最大可选规格数：1
辨识：底料 - 最大可选规格数：1
// 4. 创建【规格】内容
创建【规格】内容 -标识：双炫 - 规格分类：底料 - 价格：0
创建【规格】内容 -标识：草莓味+原味 - 规格分类：冰淇淋口味 - 价格：0
// 5. 创建【规格分类】内容
创建【规格分类】内容 - 标识：加料（多选，可重复） - 最大可选规格：2
// 6. 创建【规格】内容
创建【规格】内容 - 标识:玫果冻￥1 - 规格分类:加料（多选，可重复）- 价格:1
创建【规格】内容 - 标识:椰果￥1 - 规格分类:加料（多选，可重复）- 价格:1
创建【规格】内容 - 标识:珍珠￥1 - 规格分类:加料（多选，可重复）- 价格:1
// 7.编辑商品中原来创建的摩天脆脆 添加规格分类和规格
规格分类中添加：加料（多选，可重复、冰淇淋口味、底料 
规格选择：双炫、草莓味+原味
// 8. 创建第二个商品
创建【商品】内容 - 名称：草莓摇摇奶昔 - 商品简介：冰淇凌配草莓、一口春天味 - 图片：(暂时随意) -价格：6 - 商品分类：奶昔
// 9. 创建【规格分类】内容
标识:加料 - 最大可选规格：1
// 商品只需要只要他有多少个规格分类就行，不需要具体的规格，规格分类只需要知道他有多少种规格，规格里面做具体的事情即可
// 10.更新【规格分类】内容
标识:加料 - 最大可选规格：1 - 规格：玫果冻￥1、椰果冻￥1、珍珠￥1
标识:加料（可选，可重复） - 最大可选规格：1 - 规格：玫果冻￥1、椰果冻￥1、珍珠￥1
// 11. 更新【商品】内容
名称：草莓摇摇奶昔 - 商品简介：冰淇凌配草莓、一口春天味 - 图片：(暂时随意) -价格：6 -规格分类：加料、冰淇淋口味、底料 - 商品分类：奶昔
// 12. 创建第三个商品
创建【商品】内容 - 名称：随便的一个商品 - 商品简介：随便的一个商品 - 图片：(暂时随意) -价格：5 规格分类：加料（多选、可重复）、冰淇淋口味、底料 - 商品分类：奶昔
```

2. 模型构造完成、创建API  api/goods.js    并且将goods数据库中的数据改成所有用户可读
...
const listWithCategory = () => {
  return cloud.callFunction({
    // name里面的值，是一个云函数 data不需要，如果没有传入回调函数的话就是生成的promise
...
3. 需要使用到官方文档中的lookup来获取数据，但是不能只直接使用而是要使用到云函数

 这个是在miniprogram文件夹外面，云函数文件夹 ，参考文档

https://developers.weixin.qq.com/miniprogram/dev/wxcloud/reference-sdk-api/database/aggregate/Aggregate.lookup.html#%E7%A4%BA%E4%BE%8B

   cloud-functions/goods-list-with-category/index.js
...
4. cloud-functions/goods-list-with-category/config.json
...
5. cloud-functions/goods-list-with-category/package.json
...
<!-- 侧边栏部分和内容区域 -->
<view class="menu">
  <!-- 边侧栏组件 5.请求下来的数据传入到组件中 -->
  <siderbar list="{{ goodsList }}"></siderbar>
  <scroll-view class="menu-list" scroll-y show-scrollbar="{{ false }}" enhanced>
    <force-swiper swiperList="{{ swiperList }}"></force-swiper>
    <view class="list">
...
3. components/siderbar/index.js   接收渲染侧边栏的数据
...
4. components/siderbar/index.html   渲染侧边栏数据
...
5. components/siderbar/index.scss
...
6. components/siderbar/index.json
...
7. pages/menu/index.wxml   渲染商品数据
...
```js
1. 点击左侧侧边栏部分，然后将右侧对应的商品移至最上面
这里借助的是scroll-view组件中的一个属性：scroll-into-view
https://developers.weixin.qq.com/miniprogram/dev/component/scroll-view.html#Bug%20&%20Tip
// scroll-into-view属性
值应为某子元素id（id不能以数字开头）。设置哪个方向可滚动，则在哪个方向滚动到该元素
举例：在本次中使用
```

1. components/goods-list/index.wxml  抽离右侧部分成为一个组件（包括轮播图和商品）

componets/goods-list/index.wxml

```html
<!-- 02. scroll-into-view="section-{{current}}"   scrollInto 需要传入，在js中接收 -->
<scroll-view id="menu-list" scroll-into-view="section-{{current}}" class="menu-list" scroll-y show-scrollbar="{{ false }}" enhanced>
<!-- 将轮播图部分作为一个插槽，但是需要在js中进行配置才能使用-->
<slot name="before"></slot>
<view class="list">
  <!--实现点击左侧分类跳到相关区域 01. id="section-{{index}}"  -->
  <view  wx:for="{{ goodsList }}" wx:for-item="goodsCategory" wx:key="index" class="section" id="section-{{index}}">
```

2. componets/goods-list/index.js     接收父组件传入的 current（current是侧边栏部分的index）
...
3.  pages/menu/index.js     获取侧边栏切换时候的index，并且存入currentCategoryIndex中，等待传入组件 goods-list中作为current
...
4.  pages/menu/index.wxml        将currentCategoryIndex作为 current传入到组件goods-list中

```html
<!-- 侧边栏部分和内容区域 -->
<view class="menu">
  <!-- 边侧栏组件 5.请求下来的数据传入到组件中 bind:on-change="onSideBarChange" 监听siderbar的变化 -->
  <siderbar goodsList="{{ goodsList }}" bind:on-change="onSideBarChange"></siderbar>
  <!-- 引入goods-list组件引入，并且将需要的数据传入，抽离组件不会很臃肿 -->
  <!-- 06. 实现种类切换，传给子组件 current，是选中侧边栏部分的index -->
  <gooods-list goodsList="{{ goodsList }}" current="{{currentCategoryIndex}}">
  <!-- 使用插槽包裹轮播图 -->
  <view slot="before">
    <force-swiper swiperList="{{ swiperList }}"></force-swiper>
  </view>
...
```js
2. 实现点击左侧分类，将目标的分类移动至最顶部，和滚动右侧商品的时候，滚动到不同区域的时候，对左侧sidebar的样式进行达到切换的效果
```

##### 实现侧边栏切换和滚动切换侧边栏样式部分 全部代码 从01开始

###### 商品组件部分

1. components/goods-list/index.wxml
...
2. components/goods-list/index.js
...
Component({
  ...
  data: {
    index:0,        // 定义一个状态判断是否要加 fixed 属性(fixed吸顶的效果没有实现)
  },
  methods: {
...
3. components/goods-list/index.scss
...
4. components/goods-list/index.json
...
7. pages/menu/index.scss  没有内容仅样式
...
```json
{
  "usingComponents": {
    ...
    "siderbar":"../../components/siderbar/index",
    "force-swiper":"../../components/force-swiper/index",
    "gooods-list":"../../components/goods-list/index"
  },
  "navigationStyle": "custom"
}
```

###### 侧边栏部分 接收父组件传入滚动之后的 index 切换样式

9. components/sidebar/index.js
...
10. components/sidebar/index.wxml   没有相关内容仅布局和样式
...
11. components/sidebar/index.scss
...
12. components/sidebar/index.json
